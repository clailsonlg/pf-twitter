defmodule Pfu.UserToken do
  ...
  def build_session_token(user) do
    ...
    {token, %Pfu.UserToken{token: token, context: "session", user_id: user.id}}
  end
  ...
  def token_and_context_query(token, context) do
    from Pfu.UserToken, where: [token: ^token, context: ^context]
  end
  ...
  def user_and_contexts_query(user, :all) do
    from t in Pfu.UserToken, where: t.user_id == ^user.id
  end

  def user_and_contexts_query(user, [_ | _] = contexts) do
    from t in Pfu.UserToken, where: t.user_id == ^user.id and t.context in ^contexts
  end
end
